...
class Loader:
    ...
    def __init__(self, folder_path: str, train_ratio: float = 0.8):
        ...
        features_train, features_val, self.labels_train, self.labels_val = train_test_split(
            jumps, labelstype, train_size=train_ratio, shuffle=True
        )
        ...
        self.val_dataset = tf.data.Dataset.from_tensor_slices(
            (
                {
                    "temporal_input": self.temporal_features_test,
                    "scalar_input": self.scalar_features_test,
                },
                self.labels_val,
            )
        ).batch(16)
        ...
        ) = train_test_split(jumps_success, labelssuccess, train_size=train_ratio, shuffle=True)
        ...
        self.val_dataset_success = tf.data.Dataset.from_tensor_slices(
            (
                {
                    "temporal_input": self.temporal_features_test_success,
                    "scalar_input": self.scalar_features_test_success,
                },
                self.labels_val_success,
            )
        ).batch(16)
