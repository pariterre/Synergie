...
class StopingPage:
    def __init__(self, device : DotDevice, db_manager : DatabaseManager) -> None:
        ...
        label = ttkb.Label(self.frame, text=f"Arrêtez l'enregistrement du capteur {self.device_tag}", font=Font(self.window, size=20, weight=BOLD))
    ...
    def stopRecord(self):
        recordStopped = self.device.stop_recording()
        self.frame.destroy()
        self.frame = ttkb.Frame(self.window)
        if recordStopped :
            message = f"Enregistrement stoppé sur le capteur {self.device_tag}"
        else : 
    ...
    def stopRecordAndExtract(self):
        ...
        self.window.update()
        time.sleep(1)
        self.text.set(f"Extraction des données du capteur {self.device_tag} \nNe pas deconnecter ce capteur")
        self.label.update()
        ...
        self.checkFinish()
        if recordStopped :
            threading.Thread(target=self.device.export_data, args=([_save_data_to_file, self.extractEvent]),daemon=True).start()
        else:
