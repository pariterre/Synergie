    ...
    if "-t" in sys.argv:  # train
        ...
        if train_name == "type":
            ...
            trainer = training.Trainer(
                dataset, model.transformer(dropout=0.3, mlp_dropout=0.1), constants.filepath_model_type
            )
            trainer.train(epochs=10)
        elif train_name == "success":
            dataset = Loader(path).get_success_data()
            trainer = training.Trainer(dataset, model.lstm(), constants.filepath_model_success)
